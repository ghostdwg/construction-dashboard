...
export default function ImportSubsPage() {
  ...
  if (result) {
    return (
      <div className="max-w-4xl mx-auto py-10 px-4">
        <Link href="/subcontractors" className="text-sm text-zinc-500 hover:underline">← Subcontractors</Link>
  ...
  if (preview) {
    ...
    return (
      <div className="max-w-7xl mx-auto py-10 px-4">
        ...
        {/* Summary banner */}
        <div className="rounded-md border border-zinc-200 bg-zinc-50 px-5 py-3 mb-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
          ...
          <span className="text-zinc-700">
            <span className="font-semibold text-zinc-500">{willSkip}</span> skip
          </span>
          <span className="text-zinc-700">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            ...
            className="rounded border border-amber-300 bg-amber-50 px-3 py-1 text-xs text-amber-700 hover:bg-amber-100"
          >
            ★ Mark all preferred
          </button>
          <button
            onClick={() => setAllPreferred(false)}
            className="rounded border border-zinc-300 px-3 py-1 text-xs text-zinc-600 hover:border-zinc-500"
          >
            Clear all preferred
          </button>
          <span className="border-l border-zinc-300 mx-1" />
          <button
        ...
        {/* Preview table */}
        <div className="rounded-md border border-zinc-200 overflow-x-auto mb-6">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200 text-left text-[10px] font-semibold text-zinc-500 uppercase tracking-wide">
                  ...
                  <tr key={idx} className={row.action === "skip" ? "opacity-50" : ""}>
                    <td className="px-3 py-2">
                      <select
                        ...
                        className="rounded-md bg-white border border-zinc-300 px-2 py-1 text-xs text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                    ...
                    <td className="px-3 py-2">
                      <div className="font-medium text-zinc-800">{row.company}</div>
                      {row.conflictWith && (
                        <div className="text-[10px] text-blue-600 mt-0.5">EXISTS — {row.conflictWith.company}</div>
                      )}
                    </td>
                    <td className="px-3 py-2 text-zinc-500">{row.office ?? "—"}</td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        {row.isUnion && <span className="rounded bg-blue-100 text-blue-700 px-1 py-0.5 text-[9px] font-semibold">UNION</span>}
                        {row.isMWBE && <span className="rounded bg-purple-100 text-purple-700 px-1 py-0.5 text-[9px] font-semibold">MWBE</span>}
                      </div>
                    </td>
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4">
      ...
      <form onSubmit={handleUpload} className="rounded-md border border-zinc-200 bg-white p-6 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="rounded-md bg-white border border-zinc-300 px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 file:mr-3 file:rounded file:border-0 file:bg-zinc-100 file:px-3 file:py-1 file:text-xs file:text-zinc-700"
          />
          {file && (
            <p className="text-xs text-zinc-500 mt-1">{file.name} ({(file.size / 1024).toFixed(1)} KB)</p>
          )}
        </div>
      ...
      <div className="mt-6 rounded-md border border-blue-100 bg-blue-50 px-4 py-3 text-xs text-blue-800">
