  ...
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${
        TIER_COLORS[tier] ?? "bg-zinc-100 text-zinc-600"
      }`}
    >
      {tier}
    </span>
    ...
    <section className="mb-8">
      ...
      {editing ? (
        <div className="rounded-md border border-zinc-200 p-4 flex flex-col gap-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <label htmlFor="doNotUse" className="text-xs font-medium text-zinc-700 cursor-pointer">
                Do Not Use
              </label>
              {doNotUse && (
                <input
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="text-sm px-3 py-1.5 rounded border border-zinc-200 hover:bg-zinc-50"
            >
              Cancel
            </button>
          </div>
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          <div>
            <dt className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-0.5">Tier</dt>
            <dd><TierBadge tier={tier} /></dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-0.5">Region</dt>
            <dd className="text-zinc-700">{region || "—"}</dd>
          </div>
          <div className="col-span-2">
            <dt className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-1">Project Types</dt>
            <dd>
              {projectTypePills.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-1">
                  {projectTypePills.map((p) => (
                    <span
                      key={p}
                      className="rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs text-zinc-600 capitalize"
                    >
                      {p}
                    </span>
                  ))}
          ...
          {internalNotes && (
            <div className="col-span-2">
              <dt className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-0.5">
                Internal Notes
              </dt>
              <dd className="text-zinc-600 whitespace-pre-wrap">{internalNotes}</dd>
            </div>
          )}
        </dl>
      )}
