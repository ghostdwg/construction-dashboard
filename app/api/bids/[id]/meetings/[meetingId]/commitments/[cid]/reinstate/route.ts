// Module OPS7 — reinstate a commitment (DISMISSED → PROPOSED).
//
// POST …/commitments/[cid]/reinstate
// Any authenticated user (session wall + requireBidAccess); audited.

import { auth } from "@/lib/auth";
import { requireBidAccess } from "@/lib/auth-helpers";
import { reinstateCommitment } from "@/lib/services/meetings/commitments";
...
export async function POST(
  ...
  const result = await reinstateCommitment(bidId, mId, commitmentId, {
    name: su?.name ?? null,
    email: su?.email ?? null,
  });
  if (!result.ok) {
