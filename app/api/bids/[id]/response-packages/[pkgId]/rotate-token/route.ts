import { rotateResponsePackageToken } from "@/lib/services/tradeResponse/packages";
...
type Body = { contractorEmail?: string | null; expiresAt?: string | null };
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; pkgId: string }> }) {
  ...
  const result = await rotateResponsePackageToken(ctx.bidId, pid, {
    ...json.value,
  ...
  // rawToken is intentionally returned once by this rotation response and is never persisted.
