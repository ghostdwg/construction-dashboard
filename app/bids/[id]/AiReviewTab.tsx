...
const SOURCE_STYLES: Record<string, string> = {
  SPEC_BOOK: "bg-blue-50 text-blue-600",
  DRAWINGS: "bg-purple-50 text-purple-600",
  BOTH: "bg-indigo-50 text-indigo-600",
  BRIEF: "bg-zinc-100 text-zinc-500",
};
...
function FindingCard({
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-white p-4 flex flex-col gap-2">
      {/* Badges */}
      <div className="flex flex-wrap items-center gap-1.5">
        {finding.severity && (
          <span
            className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${SEVERITY_STYLES[finding.severity] ?? "bg-zinc-100 text-zinc-500"}`}
          >
            {finding.severity}
          </span>
        )}
        {finding.sourceDocument && SOURCE_LABELS[finding.sourceDocument] && (
          <span
            className={`rounded-full px-2 py-0.5 text-xs font-medium ${SOURCE_STYLES[finding.sourceDocument] ?? "bg-zinc-100 text-zinc-500"}`}
          >
      ...
      {/* Description */}
      {finding.title && finding.findingText && (
        <p className="text-sm text-zinc-600 leading-relaxed">{finding.findingText}</p>
      )}

      {/* Source ref */}
      {finding.sourceRef && (
        <p className="text-xs text-zinc-400 italic">Ref: {finding.sourceRef}</p>
      )}

      {/* Suggested question */}
      {finding.reviewNotes && (
        <div className="border-t border-zinc-100 pt-2 mt-0.5 flex items-start justify-between gap-3">
          <p className="text-xs text-zinc-500 italic leading-relaxed flex-1">
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            {lastAnalyzedAt && hasFindings && (
              <p className="text-xs text-zinc-400 mt-0.5">Last analyzed: {lastAnalyzedAt}</p>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="rounded bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed"
            >
      ...
      {tradeNames.length > 0 && !hasFindings && approvedEstimateCount === 0 && (
        <div className="rounded-md border border-zinc-200 bg-zinc-50 px-5 py-6 flex flex-col gap-3">
          <p className="text-sm text-zinc-500">
      ...
      {tradeNames.length > 0 && !hasFindings && approvedEstimateCount > 0 && (
        <div className="rounded-md border border-zinc-200 bg-zinc-50 px-5 py-4 flex items-center justify-between gap-4">
          ...
          <button
            onClick={runAnalysis}
            disabled={running}
            className="shrink-0 rounded bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-700 disabled:opacity-40"
          >
            {running ? "Analyzing…" : "Run Analysis"}
          </button>
        </div>
      ...
      {hasFindings && tradeNames.length > 0 && (
        <div className="flex flex-col gap-4">
          ...
          {activeTrade && (
            <div>
              {tradeNames.length === 1 && (
                <h3 className="text-sm font-semibold text-zinc-700 mb-3">{activeTrade}</h3>
              )}
              <TradePanel
