...
- **Source-proven route facts from the Q06a audit (Sprint 1, binding for this run; automation control updated by Q03.2b — see ADR 0003):** only the drawings upload carries provider-bound automation (two call sites, both smoke-gated; assert `automationStatus` on the upload response) — and that automation is master-gated by the GLOBAL persisted Admin setting **"Document AI Enrichment" (Settings → AI Configuration), default OFF**, effective on the next document action with no rebuild/recreate. It is enabled ONLY through the authenticated Admin UI — never an env var; the old `DOCUMENT_AUTOMATION_ENABLED` variable is dead and can never enable automation. `DOCUMENT_AUTOMATION_HARD_DISABLED=true` (exact literal only) is the emergency platform safety lock: it always forces automation OFF and can never enable anything. When automation does not fire, the response honestly reports `automationStatus: "disabled"` (or `"hard_disabled"`); smoke suppression still takes precedence when engaged — a storage-smoke run proves storage mechanics and suppression only, never real AI execution. Addendums and estimates uploads are clean — BUT the addendums **DELETE** route fires `triggerBriefRefresh`, so it carries its own gate (`x-addendums-storage-smoke` marker; assert response header `X-Automation-Status: suppressed_for_storage_smoke` on cleanup; note the delete success shape is 200 `{deleted:true}`, not 204). **Estimates have NO delete route anywhere in the app** — the smoke run cannot self-clean an EstimateUpload; the helper skips cleanup with a loud warning, and the leftover synthetic estimate is removed by the operator afterward (or accepted as a labeled fixture). Helper: `scripts/artifacts-staging-smoke.mjs --domain drawings|addendums|estimates` (dry-run default, `--execute --storage-only --cookie-prompt`).
