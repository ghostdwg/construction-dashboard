...
`schedule_intelligence.generate_schedule_intelligence`
...
extends the Option-A decision to schedule_intelligence.py specifically).

Before this migration, generate_schedule_intelligence() read
os.getenv("ANTHROPIC_API_KEY") directly and passed api_key= straight into
ai_gateway.create_message(). It is invoked from sidecar/routers/parse.py's
POST /parse/schedule/generate, an async background-job route matching the
precedent set by submittal_intelligence.generate_submittal_intelligence() /
/parse/submittals/generate: the credential is resolved once, TS-side
(app/api/bids/[id]/schedule-v2/generate/route.ts, via getSetting()),
forwarded as `api_key` in the initial sidecar request body, and threaded
through the background asyncio task (_run_schedule_generate) into this
function as a plain function argument — never written into the in-memory
_jobs[job_id] record. Like the submittal_intelligence.py precedent, this job
type has no callback_url / callback payload at all (results are retrieved by
polling GET /parse/schedule/status/{job_id}), so there is no callback-leak
surface to guard here.

`test_schedule_intelligence.py` (the pre-existing fidelity suite) already
covers request/response/retry/parsing fidelity in depth and has been updated
...
Runnable two ways:
  * plain stdlib:  python3 sidecar/services/__tests__/test_schedule_intelligence_credential.py
  * pytest:        pytest sidecar/services/__tests__/test_schedule_intelligence_credential.py
"""
import json
import sys
import types
...
SENTINEL = "sk-test-sentinel-do-not-use-schedule"
...
class _FakeMessage:
    def __init__(self, text, i, o, model="claude-sonnet-4-6"):
...
class _FakeAnthropicClient:
    def __init__(self, response_text=BODY):
        self.calls = []
...
def test_sentinel_traverses_to_build_client_exactly():
    ...
    try:
        out = si.generate_schedule_intelligence(SAMPLE_SECTIONS, None, "sonnet", api_key=SENTINEL)
    finally:
...
def test_sentinel_never_leaks_into_a_thrown_error_message():
    ...
    raised = None
    try:
        si.generate_schedule_intelligence(SAMPLE_SECTIONS, None, "sonnet", api_key=SENTINEL)
...
def test_sentinel_never_leaks_into_simulated_jobs_dict_or_callback_payload():
    """Mirrors what sidecar/routers/parse.py's _run_schedule_generate does:
    ...
    original = _patched_build_client(fake)
    try:
        result = si.generate_schedule_intelligence(SAMPLE_SECTIONS, None, "sonnet", api_key=SENTINEL)
    finally:
        _restore_build_client(original)
...
def test_build_client_is_the_single_construction_seam_untouched_by_this_migration():
    ...
    # untouched by this migration — schedule_intelligence.py only changed
...
if __name__ == "__main__":
    ...
    print(f"\ntest_schedule_intelligence_credential: {'PASS' if failures == 0 else str(failures) + ' FAILURE(S)'}")
