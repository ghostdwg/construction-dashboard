...
| Deterministic (regex) submittal seeding | `lib/services/submittal/seedSubmittalRegister.ts` — idempotent, defensive, additive (header `:3-9`) | **Current bug/limitation `[V]`: the seeder never writes `source`**, so its rows take the schema default `"manual"` (`schema.prisma:789 @default("manual")`) — and the AI-regeneration wipe (`generateFromAiAnalysis.ts:149`, `source in ["ai_extraction","regex_seed","csi_baseline","ai_organized"]`) and organize-ai replacement set (`organizeWithAi.ts:371`) both MISS that default, so regex-seeded rows masquerade as manual, survive auto-row replacement, and can duplicate (matches CAPABILITY-MATRIX-2026-07). The spine's `extractionMethod` must be written explicitly by every producer — never defaulted |
...
| Submittal register + provenance fields | `prisma/schema.prisma:776-848` — `source :789` (`"manual" | "regex_seed" | "ai_extraction"` per `:788`), `specSectionId :780`, `sourceJobId :816`, `linkedActivityId :820`, backward-math due dates `:819-826` | The closest thing to the target lifecycle that exists |
...
| Extraction provenance | `SubmittalItem.source` 3-value string (`:788-789`); `ScheduleActivityV2.aiGenerated/aiConfidence` (`:989-990`) | unified `extractionMethod` + `extractorVersion` + `confidence` on ONE table |
| Job attribution | `SubmittalItem.sourceJobId` (`:816`) | same pattern on candidate |
