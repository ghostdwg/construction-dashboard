...
| Submittal register + provenance fields | `prisma/schema.prisma:776-848` — `source :797` (six known values, documented inline `:788-796`: `"manual" | "regex_seed" | "ai_extraction" | "csi_baseline" | "ai_organized" | "drawing_analysis"`), `specSectionId :780`, `sourceJobId :816`, `linkedActivityId :820`, backward-math due dates `:819-826`. Since WP1b (`0ce2604`), `source` renders as a display-only provenance badge in the packages grid (`lib/services/submittal/provenanceLabels.ts` + `SubmittalsTab.tsx`) — origin only, never a quality/verification claim. | The closest thing to the target lifecycle that exists |
