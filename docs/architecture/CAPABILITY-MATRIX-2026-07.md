...
| 13 | Phase 0 IA & terminology (added 2026-07-11) | LOCAL-CODE-BUILT (copy-contract tested; NOT deployed, NOT staging-proven; no schema change, no migration) | Presentation/navigation-only slice on branch `gwx/ops-next-operational-polish`: sidebar regrouped to Overview · Pursuit · Coordination (Startup [key `handoff`] / Meetings / Action Items [key `tasks`] / Submittals / Schedule / Briefing / Procore) · Field (Register [key `operations`]) · Closeout · Reference (Warranties/Training/Inspections requirement views); Tasks page reframed in place as "Action Items" (project-wide execution queue); register heading "Operations Register" + ratified subtitle; "tracked item" prose → "Register Item(s)"; Handoff packet copy → "Startup Packet"; Field Reports defined as contractor-authored field records; stale cross-link crutch copy removed. ZERO route/query/API/schema/permission changes — all tab keys byte-identical; the known per-bid COMPLETED/CLOSED status defect is explicitly NOT fixed here (separate bug card); MeetingsTab promote copy corrected in a follow-up commit (button "Promote to Register Item"; the success toast and tooltip previously pointed at the retired Construction → Operations path, now Field → Register) |

Deploy note (operator-reported, 2026-07-11): staging runs image `groundworx-app:5403ea4-ops-polish-v1` (integration @ `5403ea4` = rows 8–12) with OPS1/OPS2 migrations applied and health green — per operator report, not independently verified here. No acceptance walkthrough upgrade has been recorded, so no row is upgraded past LOCAL-CODE-BUILT/deployed; nothing OPS-related is staging-PROVEN and no `[OP]` lifecycle claim is made.
