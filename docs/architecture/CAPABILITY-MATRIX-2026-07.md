...
**Staging-proven vs unit-tested `[OP]`/`[V]`** — The FULL Spec Book storage lifecycle (upload → sidecar parse → split → serve section PDF → delete → re-upload → cleanup) is the ONLY staging-proven lifecycle: 13/13 smoke steps, automation suppressed (Ledger §1). Drawings/estimates/addendums/meetings durable writes are code+unit-tests only, never run live; meetings durability-read is unprovable safely today (read path triggers sidecar re-transcription; Ledger §1). No audio-availability probe exists at this SHA (grep = zero) — that work lives on a different branch.
...
| 5 | Spec Book storage / splitting | VERIFIED-WORKING (storage only) | The 13/13 staging-proven lifecycle covers storage mechanics with AI suppressed; the analyze lane (`aiExtractions`) that feeds every downstream register is UNPROVEN; non-specbook lifecycles never run live |
